...
<html><head>
  ...
  <style>
    body {% raw %}{{{% endraw %} font: 14px/1.5 system-ui, sans-serif; margin: 0; background: #1a1a2e; color: #eee; {% raw %}}}{% endraw %}
    .container {% raw %}{{{% endraw %} display: flex; height: 100vh; {% raw %}}}{% endraw %}
    .sidebar {% raw %}{{{% endraw %} width: 280px; background: #252542; padding: 20px; border-right: 1px solid #3d3d5c; overflow-y: auto; flex-shrink: 0; {% raw %}}}{% endraw %}
    .main {% raw %}{{{% endraw %} flex: 1; padding: 20px; overflow-y: auto; {% raw %}}}{% endraw %}
    h1 {% raw %}{{{% endraw %} margin: 0 0 10px 0; font-size: 18px; {% raw %}}}{% endraw %}
    h2 {% raw %}{{{% endraw %} margin: 20px 0 10px 0; font-size: 14px; color: #888; text-transform: uppercase; {% raw %}}}{% endraw %}
    .stat {% raw %}{{{% endraw %} display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #3d3d5c; {% raw %}}}{% endraw %}
    .stat-value {% raw %}{{{% endraw %} font-weight: bold; {% raw %}}}{% endraw %}
    .bar-row {% raw %}{{{% endraw %} display: flex; align-items: center; margin: 6px 0; {% raw %}}}{% endraw %}
    .bar-label {% raw %}{{{% endraw %} width: 55px; font-size: 12px; color: #aaa; {% raw %}}}{% endraw %}
    .bar {% raw %}{{{% endraw %} height: 18px; border-radius: 3px; {% raw %}}}{% endraw %}
    .bar-pct {% raw %}{{{% endraw %} margin-left: 8px; font-size: 12px; color: #666; {% raw %}}}{% endraw %}
    .tree {% raw %}{{{% endraw %} list-style: none; padding-left: 20px; {% raw %}}}{% endraw %}
    details {% raw %}{{{% endraw %} cursor: pointer; {% raw %}}}{% endraw %}
    summary {% raw %}{{{% endraw %} padding: 4px 8px; border-radius: 4px; {% raw %}}}{% endraw %}
    summary:hover {% raw %}{{{% endraw %} background: #2d2d44; {% raw %}}}{% endraw %}
    .folder {% raw %}{{{% endraw %} color: #ffd700; {% raw %}}}{% endraw %}
    .file {% raw %}{{{% endraw %} display: flex; align-items: center; padding: 4px 8px; border-radius: 4px; {% raw %}}}{% endraw %}
    .file:hover {% raw %}{{{% endraw %} background: #2d2d44; {% raw %}}}{% endraw %}
    .size {% raw %}{{{% endraw %} color: #888; margin-left: auto; font-size: 12px; {% raw %}}}{% endraw %}
    .dot {% raw %}{{{% endraw %} width: 8px; height: 8px; border-radius: 50%; margin-right: 8px; {% raw %}}}{% endraw %}
  </style>
</head><body>
  <div class="container">
    <div class="sidebar">
  ...
  <script>
    ...
    function fmt(b) {% raw %}{{{% endraw %} if (b < 1024) return b + ' B'; if (b < 1048576) return (b/1024).toFixed(1) + ' KB'; return (b/1048576).toFixed(1) + ' MB'; {% raw %}}}{% endraw %}
    function render(node, parent) {% raw %}{{{% endraw %}
      if (node.children) {% raw %}{{{% endraw %}
        const det = document.createElement('details');
        det.open = parent === document.getElementById('root');
        det.innerHTML = `<summary><span class="folder">📁 ${% raw %}{{{% endraw %}node.name{% raw %}}}{% endraw %}</span><span class="size">${% raw %}{{{% endraw %}fmt(node.size){% raw %}}}{% endraw %}</span></summary>`;
      ...
      {% raw %}}}{% endraw %} else {% raw %}{{{% endraw %}
        ...
        li.innerHTML = `<span class="dot" style="background:${% raw %}{{{% endraw %}colors[node.ext]||'#6b7280'{% raw %}}}{% endraw %}"></span>${% raw %}{{{% endraw %}node.name{% raw %}}}{% endraw %}<span class="size">${% raw %}{{{% endraw %}fmt(node.size){% raw %}}}{% endraw %}</span>`;
